...
def test_bar_state_v2_postgresql_release_gate(tmp_path: Path) -> None:
    ...
    assert tuple(item.version for item in migrations) == tuple(range(1, 31))
    ...
    assert first.applied == tuple(range(1, 31))
    ...
    assert repeated.skipped == tuple(range(1, 31))
    ...
    assert latest == {
        "version": 30,
        "name": "m0b_numeric_admission_worker_api",
        "checksum": migration_by_version[30].checksum,
    }
...
def test_bar_state_v2a_postgresql_release_gate(tmp_path: Path) -> None:
    ...
    assert latest["name"] == "m0b_numeric_admission_worker_api"
